            ...
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              ...
              <nav className="mt-5 flex-1 px-2 bg-color space-y-1">
                ...
                {routes?.map((route, key) => {
                  const _style = route.path === pathname ? "bg-blue-100 text-blue-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md" : "text-white hover:bg-blue-50 hover:text-blue-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md"
                  const _style_icon = route.path === pathname ? "text-blue-500 mr-3 flex-shrink-0 h-6 w-6" : "text-white group-hover:text-blue-500 mr-3 flex-shrink-0 h-6 w-6"
                  const Icon = route.icon ? route : <svg />
                  return (
                    <Link
                      to={route?.path}
                      className={_style}
                    >
                      {route?.icon && route?.icon({ className: _style_icon })}
                      {route?.name}
                    </Link>
                  )
                })}
